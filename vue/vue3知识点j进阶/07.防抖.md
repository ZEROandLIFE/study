# Vue 3 防抖完全指南 ⏱️

## 🎯 防抖在 Vue 3 中的核心概念

**防抖（Debounce）**：高频事件触发时，只执行最后一次，忽略中间过程。

## 🔧 6 种 Vue 3 防抖实现方式

### 1. **Composition API 原生实现**
```javascript
// useDebounce.js
import { ref, onUnmounted } from 'vue'

export function useDebounce(fn, delay = 300) {
  let timer = null
  const debouncedFn = (...args) => {
    if (timer) clearTimeout(timer)
    timer = setTimeout(() => {
      fn(...args)
    }, delay)
  }
  
  // 清理函数
  const cancel = () => {
    if (timer) {
      clearTimeout(timer)
      timer = null
    }
  }
  
  // 立即执行版本
  const debounceImmediate = (fn, delay = 300) => {
    let timer = null
    return (...args) => {
      if (!timer) fn(...args)  // 首次立即执行
      if (timer) clearTimeout(timer)
      timer = setTimeout(() => {
        timer = null
      }, delay)
    }
  }
  
  onUnmounted(cancel)
  
  return {
    debouncedFn,
    cancel,
    debounceImmediate
  }
}
```

### 2. **组件内直接使用**
```js
<script setup>
import { ref } from 'vue'

const searchInput = ref('')
let timer = null

// 基本防抖
const handleSearch = (value) => {
  if (timer) clearTimeout(timer)
  timer = setTimeout(() => {
    console.log('搜索:', value)
    // API调用
  }, 500)
}

// 带参数的防抖
const debounce = (fn, delay = 300) => {
  let timer = null
  return (...args) => {
    if (timer) clearTimeout(timer)
    timer = setTimeout(() => {
      fn.apply(this, args)
    }, delay)
  }
}

// 使用
const searchApi = (keyword) => {
  console.log('调用API:', keyword)
}

const debouncedSearch = debounce(searchApi, 500)
</script>

<template>
  <input 
    v-model="searchInput" 
    @input="(e) => handleSearch(e.target.value)"
    placeholder="输入搜索..."
  />
  
  <!-- 或者 -->
  <input 
    @input="(e) => debouncedSearch(e.target.value)"
  />
</template>
```

### 3. **响应式防抖（与 ref 结合）**
```js
<script setup>
import { ref, watch, computed } from 'vue'

// 1. watch 中使用防抖
const searchKeyword = ref('')

watch(
  searchKeyword,
  debounce((newVal) => {
    if (newVal.trim()) {
      fetchResults(newVal)
    }
  }, 500)
)

// 2. computed 防抖（复杂计算时）
const expensiveValue = computed(() => {
  // 复杂计算...
  return heavyCalculation()
})

const debouncedValue = ref('')
watch(
  expensiveValue,
  debounce((newVal) => {
    debouncedValue.value = newVal
  }, 300)
)

// 3. 自定义防抖 ref
function useDebouncedRef(initialValue, delay = 300) {
  const value = ref(initialValue)
  let timer = null
  
  const setValue = (newValue) => {
    if (timer) clearTimeout(timer)
    timer = setTimeout(() => {
      value.value = newValue
    }, delay)
  }
  
  return {
    value,
    setValue
  }
}

// 使用
const debouncedInput = useDebouncedRef('', 500)
</script>
```

### 4. **自定义指令实现**
```javascript
// directives/debounce.js
export const vDebounce = {
  mounted(el, binding) {
    const { value: fn, arg: delay = 300 } = binding
    let timer = null
    
    const handler = (event) => {
      if (timer) clearTimeout(timer)
      timer = setTimeout(() => {
        fn(event)
      }, delay)
    }
    
    // 存储 handler 用于清理
    el._debounceHandler = handler
    
    // 监听事件（支持多个事件）
    const events = binding.modifiers.click ? ['click'] : 
                   binding.modifiers.input ? ['input'] : ['input']
    
    events.forEach(eventName => {
      el.addEventListener(eventName, handler)
    })
  },
  unmounted(el) {
    // 清理事件监听器
    if (el._debounceHandler) {
      el.removeEventListener('input', el._debounceHandler)
      el.removeEventListener('click', el._debounceHandler)
    }
  }
}

// 全局注册
import { createApp } from 'vue'
import { vDebounce } from './directives/debounce'

const app = createApp(App)
app.directive('debounce', vDebounce)
```

```js
<!-- 使用自定义指令 -->
<template>
  <!-- 输入框防抖 -->
  <input 
    v-debounce:500="handleInput"
    placeholder="输入防抖..."
  />
  
  <!-- 点击防抖 -->
  <button 
    v-debounce.click:1000="handleClick"
  >
    防抖按钮
  </button>
  
  <!-- 带参数 -->
  <input 
    v-debounce="(e) => search(e.target.value)"
  />
</template>
```

### 5. **VueUse 的 useDebounceFn**
```js
<script setup>
import { ref } from 'vue'
import { useDebounceFn, watchDebounced } from '@vueuse/core'

// 1. useDebounceFn - 最常用
const searchKeyword = ref('')
const results = ref([])

// 创建防抖函数
const debouncedSearch = useDebounceFn(async (keyword) => {
  if (!keyword.trim()) return
  
  const data = await fetch(`/api/search?q=${keyword}`)
  results.value = data
}, 500)

// 在输入时调用
const handleInput = (e) => {
  debouncedSearch(e.target.value)
}

// 2. watchDebounced - 监听防抖
watchDebounced(
  searchKeyword,
  async (newVal) => {
    if (newVal.trim()) {
      const data = await fetch(`/api/search?q=${newVal}`)
      results.value = data
    }
  },
  { debounce: 500, maxWait: 1000 } // 最大等待时间
)

// 3. 防抖 ref
import { debouncedRef } from '@vueuse/core'
const debouncedValue = debouncedRef('', 500)
</script>
```

### 6. **高级防抖 Hook**
```javascript
// useAdvancedDebounce.js
import { ref, onUnmounted } from 'vue'

export function useAdvancedDebounce(options = {}) {
  const {
    delay = 300,
    immediate = false, // 是否立即执行
    maxWait = null,    // 最大等待时间
    trailing = true,   // 延迟结束后是否执行
    leading = false    // 延迟开始前是否执行
  } = options

  let timer = null
  let lastCallTime = 0
  let lastInvokeTime = 0
  let lastArgs = null
  let lastThis = null

  const invokeFunc = (time) => {
    const args = lastArgs
    const thisArg = lastThis
    
    lastArgs = lastThis = null
    lastInvokeTime = time
    
    return fn.apply(thisArg, args)
  }

  const startTimer = (pendingFunc, wait) => {
    timer = setTimeout(pendingFunc, wait)
  }

  const shouldInvoke = (time) => {
    const timeSinceLastCall = time - lastCallTime
    const timeSinceLastInvoke = time - lastInvokeTime
    
    return (
      lastCallTime === 0 ||
      timeSinceLastCall >= delay ||
      (maxWait !== null && timeSinceLastInvoke >= maxWait)
    )
  }

  const remainingWait = (time) => {
    const timeSinceLastCall = time - lastCallTime
    const timeSinceLastInvoke = time - lastInvokeTime
    const timeWaiting = delay - timeSinceLastCall
    
    return maxWait === null
      ? timeWaiting
      : Math.min(timeWaiting, maxWait - timeSinceLastInvoke)
  }

  const debounced = function(...args) {
    const time = Date.now()
    const isInvoking = shouldInvoke(time)
    
    lastArgs = args
    lastThis = this
    lastCallTime = time
    
    if (isInvoking) {
      if (timer === null) {
        if (leading) {
          return invokeFunc(time)
        }
      }
      
      if (maxWait !== null) {
        timer = setTimeout(() => {
          if (trailing && lastArgs) {
            return invokeFunc(Date.now())
          }
          timer = null
        }, delay)
        
        if (immediate) {
          return invokeFunc(time)
        }
      }
    }
    
    if (timer === null) {
      timer = setTimeout(() => {
        if (trailing && lastArgs) {
          return invokeFunc(Date.now())
        }
        timer = null
      }, delay)
    }
  }

  const cancel = () => {
    if (timer !== null) {
      clearTimeout(timer)
    }
    lastInvokeTime = 0
    lastArgs = lastCallTime = lastThis = timer = null
  }

  const flush = () => {
    return timer === null ? undefined : invokeFunc(Date.now())
  }

  onUnmounted(cancel)

  return {
    debounced,
    cancel,
    flush
  }
}
```

## 🎨 实际应用场景

### **场景1：搜索框防抖**
```js
<template>
  <div class="search-container">
    <!-- 方案1：直接使用 -->
    <input
      v-model="keyword"
      @input="handleSearch"
      placeholder="搜索商品..."
      class="search-input"
    />
    
    <!-- 方案2：使用自定义指令 -->
    <input
      v-debounce:500="search"
      placeholder="指令方式搜索..."
    />
    
    <!-- 方案3：使用 VueUse -->
    <input
      @input="(e) => debouncedSearch(e.target.value)"
      placeholder="VueUse 方式..."
    />
    
    <!-- 加载状态 -->
    <div v-if="loading" class="loading">
      搜索中...
    </div>
    
    <!-- 结果展示 -->
    <div v-if="results.length" class="results">
      <div 
        v-for="item in results" 
        :key="item.id"
        class="result-item"
      >
        {{ item.name }}
      </div>
    </div>
  </div>
</template>

<script setup>
import { ref } from 'vue'
import { useDebounceFn } from '@vueuse/core'

const keyword = ref('')
const results = ref([])
const loading = ref(false)

// API 调用
const searchAPI = async (query) => {
  if (!query.trim()) {
    results.value = []
    return
  }
  
  loading.value = true
  try {
    const response = await fetch(`/api/search?q=${encodeURIComponent(query)}`)
    const data = await response.json()
    results.value = data
  } catch (error) {
    console.error('搜索失败:', error)
    results.value = []
  } finally {
    loading.value = false
  }
}

// 方法1：手动防抖
let searchTimer = null
const handleSearch = () => {
  if (searchTimer) clearTimeout(searchTimer)
  searchTimer = setTimeout(() => {
    searchAPI(keyword.value)
  }, 500)
}

// 方法2：VueUse 防抖
const debouncedSearch = useDebounceFn(searchAPI, 500)

// 方法3：立即执行的防抖（首次输入立即搜索）
const immediateSearch = (query) => {
  if (!query.trim()) return
  
  if (searchTimer) clearTimeout(searchTimer)
  
  // 如果是第一次输入，立即搜索
  if (!keyword.value && query) {
    searchAPI(query)
  } else {
    searchTimer = setTimeout(() => {
      searchAPI(query)
    }, 500)
  }
}
</script>
```

### **场景2：表单输入防抖验证**
```js
<template>
  <form @submit.prevent="handleSubmit">
    <!-- 用户名实时验证 -->
    <div class="form-group">
      <label>用户名</label>
      <input
        v-model="form.username"
        @input="validateUsername"
        :class="{ 'error': usernameError }"
      />
      <div v-if="usernameError" class="error-message">
        {{ usernameError }}
      </div>
      <div v-if="validating" class="validating">
        检查用户名...
      </div>
    </div>
    
    <!-- 邮箱防抖验证 -->
    <div class="form-group">
      <label>邮箱</label>
      <input
        v-model="form.email"
        @input="debouncedValidateEmail"
        :class="{ 'error': emailError }"
      />
      <div v-if="emailError" class="error-message">
        {{ emailError }}
      </div>
    </div>
    
    <button type="submit" :disabled="isSubmitting">
      {{ isSubmitting ? '提交中...' : '提交' }}
    </button>
  </form>
</template>

<script setup>
import { ref } from 'vue'
import { useDebounceFn } from '@vueuse/core'

const form = ref({
  username: '',
  email: ''
})

const usernameError = ref('')
const emailError = ref('')
const validating = ref(false)
const isSubmitting = ref(false)

// 用户名验证（带防抖）
let validateTimer = null
const validateUsername = async () => {
  if (validateTimer) clearTimeout(validateTimer)
  
  const username = form.value.username
  
  if (!username) {
    usernameError.value = '用户名不能为空'
    return
  }
  
  if (username.length < 3) {
    usernameError.value = '用户名至少3个字符'
    return
  }
  
  // 防抖：等待用户停止输入
  validateTimer = setTimeout(async () => {
    validating.value = true
    try {
      // 检查用户名是否可用
      const response = await fetch(`/api/check-username?username=${username}`)
      const { available } = await response.json()
      
      if (!available) {
        usernameError.value = '用户名已存在'
      } else {
        usernameError.value = ''
      }
    } catch (error) {
      usernameError.value = '验证失败，请重试'
    } finally {
      validating.value = false
    }
  }, 500)
}

// 邮箱验证（使用 VueUse）
const validateEmail = async (email) => {
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  
  if (!email) {
    emailError.value = '邮箱不能为空'
    return
  }
  
  if (!emailRegex.test(email)) {
    emailError.value = '邮箱格式不正确'
    return
  }
  
  // 检查邮箱是否已注册
  try {
    const response = await fetch(`/api/check-email?email=${email}`)
    const { registered } = await response.json()
    
    if (registered) {
      emailError.value = '邮箱已注册'
    } else {
      emailError.value = ''
    }
  } catch (error) {
    emailError.value = '验证失败'
  }
}

const debouncedValidateEmail = useDebounceFn((e) => {
  validateEmail(e.target.value)
}, 500)
</script>
```

### **场景3：窗口 resize 防抖**
```js
<script setup>
import { ref, onMounted, onUnmounted } from 'vue'

const windowSize = ref({
  width: window.innerWidth,
  height: window.innerHeight
})

const deviceType = ref('desktop')

// 防抖的 resize 处理
let resizeTimer = null
const handleResize = () => {
  if (resizeTimer) clearTimeout(resizeTimer)
  
  resizeTimer = setTimeout(() => {
    windowSize.value = {
      width: window.innerWidth,
      height: window.innerHeight
    }
    
    // 根据宽度判断设备类型
    if (window.innerWidth < 768) {
      deviceType.value = 'mobile'
    } else if (window.innerWidth < 1024) {
      deviceType.value = 'tablet'
    } else {
      deviceType.value = 'desktop'
    }
    
    // 触发布局更新
    updateLayout()
  }, 200)
}

// 布局更新函数
const updateLayout = () => {
  // 根据设备类型更新布局
  switch (deviceType.value) {
    case 'mobile':
      // 移动端布局
      break
    case 'tablet':
      // 平板布局
      break
    case 'desktop':
      // 桌面布局
      break
  }
}

onMounted(() => {
  window.addEventListener('resize', handleResize)
})

onUnmounted(() => {
  window.removeEventListener('resize', handleResize)
  if (resizeTimer) clearTimeout(resizeTimer)
})
</script>

<template>
  <div :class="['container', `device-${deviceType}`]">
    <div>窗口尺寸: {{ windowSize.width }} x {{ windowSize.height }}</div>
    <div>设备类型: {{ deviceType }}</div>
  </div>
</template>

<style scoped>
.container {
  padding: 20px;
}

.device-mobile {
  max-width: 100%;
  font-size: 14px;
}

.device-tablet {
  max-width: 768px;
  font-size: 16px;
}

.device-desktop {
  max-width: 1200px;
  font-size: 18px;
}
</style>
```

## 🚀 性能优化技巧

### **1. 防抖器复用池**
```javascript
// debouncePool.js - 避免重复创建防抖器
class DebouncePool {
  constructor() {
    this.pool = new Map()
  }
  
  get(key, fn, delay = 300) {
    if (this.pool.has(key)) {
      return this.pool.get(key)
    }
    
    const debouncedFn = this.createDebounce(fn, delay)
    this.pool.set(key, debouncedFn)
    
    return debouncedFn
  }
  
  createDebounce(fn, delay) {
    let timer = null
    return (...args) => {
      if (timer) clearTimeout(timer)
      timer = setTimeout(() => {
        fn(...args)
      }, delay)
    }
  }
  
  clear(key) {
    if (this.pool.has(key)) {
      this.pool.delete(key)
    }
  }
  
  clearAll() {
    this.pool.clear()
  }
}

// 使用
const pool = new DebouncePool()

// 在组件中使用
const handleSearch = pool.get(
  'search-input', 
  (value) => searchAPI(value), 
  500
)
```

### **2. 自动清理防抖器**
```javascript
// useAutoCleanDebounce.js
import { onScopeDispose } from 'vue'

export function useAutoCleanDebounce() {
  const timers = new Set()
  
  const debounce = (fn, delay = 300) => {
    let timer = null
    
    const wrappedFn = (...args) => {
      if (timer) clearTimeout(timer)
      timer = setTimeout(() => {
        fn(...args)
      }, delay)
      
      timers.add(timer)
    }
    
    // 添加取消方法
    wrappedFn.cancel = () => {
      if (timer) {
        clearTimeout(timer)
        timers.delete(timer)
      }
    }
    
    return wrappedFn
  }
  
  // 组件卸载时自动清理所有定时器
  onScopeDispose(() => {
    timers.forEach(timer => clearTimeout(timer))
    timers.clear()
  })
  
  return { debounce }
}
```

### **3. 防抖配置策略**
```javascript
// debounceStrategies.js
export const debounceStrategies = {
  // 搜索场景：中等延迟
  SEARCH: {
    delay: 500,
    maxWait: 2000,
    immediate: false
  },
  
  // 表单验证：快速响应
  VALIDATION: {
    delay: 300,
    maxWait: 1000,
    immediate: false
  },
  
  // 窗口调整：平滑过渡
  RESIZE: {
    delay: 200,
    maxWait: 500,
    immediate: false
  },
  
  // 按钮点击：防止重复提交
  BUTTON: {
    delay: 1000,
    maxWait: 3000,
    immediate: true
  },
  
  // 实时保存：用户无感
  AUTO_SAVE: {
    delay: 1000,
    maxWait: 5000,
    immediate: false
  }
}

// 策略工厂
export function createDebounceByStrategy(strategyName, fn) {
  const strategy = debounceStrategies[strategyName] || debounceStrategies.SEARCH
  
  return useDebounceFn(fn, strategy.delay, {
    maxWait: strategy.maxWait
  })
}

// 使用
const saveData = createDebounceByStrategy('AUTO_SAVE', async (data) => {
  await api.save(data)
})
```

## 📊 性能监控与调试

### **防抖性能监控**
```javascript
// debounceMonitor.js
export function monitorDebouncePerformance() {
  const metrics = {
    calls: 0,
    executions: 0,
    savedCalls: 0,
    averageDelay: 0
  }
  
  const originalSetTimeout = window.setTimeout
  const originalClearTimeout = window.clearTimeout
  
  const timers = new Map()
  
  // 拦截 setTimeout
  window.setTimeout = function(callback, delay, ...args) {
    const id = originalSetTimeout.call(this, () => {
      metrics.executions++
      callback()
    }, delay, ...args)
    
    timers.set(id, {
      start: Date.now(),
      delay
    })
    
    return id
  }
  
  // 拦截 clearTimeout
  window.clearTimeout = function(id) {
    const timer = timers.get(id)
    if (timer) {
      const actualDelay = Date.now() - timer.start
      if (actualDelay < timer.delay) {
        metrics.savedCalls++
      }
      timers.delete(id)
    }
    
    return originalClearTimeout.call(this, id)
  }
  
  // 创建带监控的防抖
  const createMonitoredDebounce = (fn, delay) => {
    let timer = null
    
    return (...args) => {
      metrics.calls++
      
      if (timer) {
        window.clearTimeout(timer)
      }
      
      timer = window.setTimeout(() => {
        fn(...args)
      }, delay)
    }
  }
  
  return {
    createMonitoredDebounce,
    getMetrics: () => ({
      ...metrics,
      efficiency: metrics.savedCalls / metrics.calls * 100
    })
  }
}
```

### **Vue DevTools 插件**
```javascript
// vue-debounce-devtools.js
export const VueDebounceDevtools = {
  install(app, options = {}) {
    const debouncedFunctions = new Map()
    
    // 包装所有 methods 中的函数
    app.mixin({
      beforeCreate() {
        const methods = this.$options.methods
        if (methods) {
          Object.keys(methods).forEach(key => {
            const originalFn = methods[key]
            
            // 检查是否有 @debounce 装饰器
            const debounceMeta = originalFn.__debounce
            if (debounceMeta) {
              const { delay = 300 } = debounceMeta
              
              const debouncedFn = debounce(originalFn, delay)
              methods[key] = debouncedFn
              
              debouncedFunctions.set(debouncedFn, {
                component: this.$options.name || 'Anonymous',
                method: key,
                delay,
                calls: 0,
                executions: 0
              })
            }
          })
        }
      }
    })
    
    // 添加到 DevTools
    if (window.__VUE_DEVTOOLS_GLOBAL_HOOK__) {
      window.__VUE_DEVTOOLS_GLOBAL_HOOK__.on.debounce = debouncedFunctions
    }
  }
}

// 装饰器
export function Debounce(delay = 300) {
  return function(target, key, descriptor) {
    const original = descriptor.value
    descriptor.value = function(...args) {
      if (!this.__debounceTimer) {
        this.__debounceTimer = setTimeout(() => {
          original.apply(this, args)
          this.__debounceTimer = null
        }, delay)
      }
    }
    descriptor.value.__debounce = { delay }
    return descriptor
  }
}

// 使用
import { Debounce } from './vue-debounce-devtools'

export default {
  methods: {
    @Debounce(500)
    handleSearch() {
      // 防抖处理
    }
  }
}
```

## 🎯 最佳实践总结

### **防抖配置推荐值**
```javascript
const DEBOUNCE_CONFIG = {
  SEARCH_INPUT: 300,      // 搜索框
  FORM_VALIDATION: 500,   // 表单验证
  AUTO_SAVE: 1000,        // 自动保存
  WINDOW_RESIZE: 200,     // 窗口调整
  BUTTON_CLICK: 1000,     // 按钮防重
  SCROLL_EVENTS: 150,     // 滚动事件
  MOUSE_MOVE: 100,        // 鼠标移动
  REAL_TIME_SAVE: 3000    // 实时保存（长延迟）
}
```

### **组件设计模式**
```js
<!-- DebounceInput.vue - 可复用的防抖输入组件 -->
<template>
  <input
    :value="modelValue"
    @input="handleInput"
    v-bind="$attrs"
  />
</template>

<script setup>
import { useDebounceFn } from '@vueuse/core'

const props = defineProps({
  modelValue: String,
  delay: {
    type: Number,
    default: 300
  },
  immediate: {
    type: Boolean,
    default: false
  }
})

const emit = defineEmits(['update:modelValue', 'change'])

// 防抖处理
const debouncedEmit = useDebounceFn((value) => {
  emit('update:modelValue', value)
  emit('change', value)
}, props.delay)

const handleInput = (event) => {
  const value = event.target.value
  
  if (props.immediate) {
    emit('update:modelValue', value)
  }
  
  debouncedEmit(value)
}
</script>
```

### **错误处理与边界情况**
```javascript
// 健壮的防抖实现
function robustDebounce(fn, delay, options = {}) {
  const {
    maxWait = null,
    leading = false,
    trailing = true
  } = options
  
  let timer = null
  let lastCallTime = 0
  let lastInvokeTime = 0
  let lastArgs = null
  let lastThis = null
  let maxTimer = null
  
  function invokeFunc(time) {
    const args = lastArgs
    const thisArg = lastThis
    
    lastArgs = lastThis = null
    lastInvokeTime = time
    
    try {
      return fn.apply(thisArg, args)
    } catch (error) {
      console.error('防抖函数执行错误:', error)
      // 可以在这里添加错误处理逻辑
      if (options.onError) {
        options.onError(error)
      }
      throw error
    }
  }
  
  function shouldInvoke(time) {
    const timeSinceLastCall = time - lastCallTime
    const timeSinceLastInvoke = time - lastInvokeTime
    
    return (
      lastCallTime === 0 ||
      timeSinceLastCall >= delay ||
      (maxWait !== null && timeSinceLastInvoke >= maxWait)
    )
  }
  
  // ... 其余实现
}
```

## 📈 性能影响分析

| 场景                      | 无防抖       | 有防抖      | 提升 |
| ------------------------- | ------------ | ----------- | ---- |
| **搜索输入（10个字符）**  | 10次API调用  | 1次API调用  | 90%  |
| **窗口resize（调整5秒）** | 300+次重排   | 10-20次重排 | 95%  |
| **表单实时验证**          | 频繁验证阻塞 | 平滑验证    | 80%  |
| **按钮重复点击**          | 多次提交     | 单次提交    | 100% |
| **滚动加载**              | 频繁计算     | 节流计算    | 85%  |

## 🚀 Vue 3 防抖要点总结

1. **优先使用 VueUse** - `useDebounceFn` 最方便
2. **合理设置延迟** - 300ms 适用于多数场景
3. **注意内存泄漏** - 组件卸载时清理定时器
4. **区分防抖和节流** - 防抖：最后一次，节流：固定频率
5. **结合业务场景** - 搜索、验证、保存使用不同策略
6. **监控性能** - 确保防抖真正提升体验

**记住**：防抖的目的是**提升用户体验，减少不必要的计算/请求**，不是所有的输入都需要防抖。关键交互应该保持即时响应！