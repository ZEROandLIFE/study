...
## 核心要点
1. v-model 是双向绑定的语法糖，本质是 value + @input 事件的组合
2. 不同输入类型对应不同的数据类型（如复选框绑定布尔值或数组）
3. 修饰符（.lazy、.number、.trim）可以优化输入行为
4. 自定义组件可以通过 modelValue 和 update:modelValue 实现 v-model
