<template>
  <div>
    ...
    <ul v-for="item in newslist" :key="item.id">
      <!-- 这个是query传参 -->
      <!-- ：to这种不能直接拼一块，因为path就是路径，query需要单独写，直接用to接受的是字符串，所以会自动解析 -->
      <!-- <router-link :to="{path:'/news/newsdetails?a=&b=heihei'}">{{item.title}}</router-link> -->
      ...
      <!-- <router-link :to="`/news/newsdetails?id=${ item.id }&name=${ item.name }&title=${item.title}&content=${item.content}`">{{
        item.title
      }}</router-link> -->

      <!-- 这个是params传参 -->
      <!-- <router-link :to="{name:'newsdetails',params:{id: item.id}}">{{item.title}}</router-link> -->
      <!-- Vue Router 的 params 仅在通过 name 跳转时生效。 -->
    </ul>
    <h3>这是展示区域</h3>
